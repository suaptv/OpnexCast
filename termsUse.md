使用条款 / Terms of Service

## 1. 接受条款 / Acceptance of Terms
使用OpnexCast即表示您同意这些条款。 / By using OpnexCast, you agree to these terms.

## 2. 许可 / License
- 不得用于非法目的 / Not for illegal purposes.
- 遵守版权法规 / Comply with copyright regulations.

## 3. 服务说明 / Service Description
OpnexCast提供DLNA投屏功能： / OpnexCast provides DLNA casting features:
- 本地相册媒体投屏 / Local media casting from album.
- 网络媒体浏览和投屏 / Network media browsing and casting.
- DLNA设备发现和连接 / DLNA device discovery and connection.

## 4. 用户责任 / User Responsibilities
- 确保拥有投屏内容的合法权利 / Ensure you have legal rights to the content being cast.
- 遵守当地法律法规 / Comply with local laws and regulations.
- 负责您的设备和网络安全 / Responsible for your device and network security.

## 5. 免责声明 / Disclaimer
- 应用按"现状"提供 / The app is provided "as is".
- 不保证兼容所有DLNA设备 / No guarantee of compatibility with all DLNA devices.
- 网络功能依赖于网络状况 / Network functionality depends on network conditions.

## 6. 知识产权 / Intellectual Property
OpnexCast及其内容受版权保护。 / OpnexCast and its content are protected by copyright.

## 7. 终止 / Termination
我们保留随时终止或修改服务的权利。 / We reserve the right to terminate or modify the service at any time.

## 8. 联系方式 / Contact Information
如有问题，请通过应用内"联系我们"功能反馈。 / If you have any questions, please use the "Contact Us" feature within the app.

最后更新：2026年1月18日 / Last updated: January 18, 2026
